...
function HistoryPlugin (schema, options = {}) {
  ...
  schema.pre(/deleteOne|remove|deleteById/, async function () {
...
async function mountDocToSave ({ options, params, doc }, method) {
  ...
  if ({}.hasOwnProperty.call(self, 'isNew') || self.isNew) {
  ...
  if (method === 'updated') {
    const tmpResult = await _helperPreUpdate.call(this, result)
    if (tmpResult) {
      result = tmpResult
    }
  }
